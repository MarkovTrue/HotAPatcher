...
**Настройки новой игры не запоминаются**. Список городов приходится выставлять заново каждый раз. Полный случайный выбор мне не подходит, потому что есть города, которые я не люблю. Пропатченная игра сохраняет выбранные город, героя и стартовый бонус в момент генерации карты в `HotaPatcher.dat`. Можно безопасно удалить этот файл, тогда всё вернётся в состояние «Случайно».
